...
/*
 * This function takes a signal name and mangles it into an equivalent name that is suitable to the EDIF format.
 */
 ...
 * Beware!
 *
 * I traded off convenience for simplicity (static buffer) mangle_edif_name() should not be called more then once as an
 * argument to a function and should not be called until you are done using a buffer. If you really need to work with
 * several mangled names simultaneously create several char * variables and strdup mangle return values into them.
 */
...
void start_net(ivl_nexus_t nex) {
	fprintf(out, "\t\t\t\t(net (rename %s \"%s\") (joined\n", mangle_edif_name(ivl_nexus_name(nex)), ivl_nexus_name(nex));
}
		...
		case IVL_LO_AND:
			...
			do {
				for (i = 0; i < level_pins / 2; i++) {
					fprintf(out, "\t\t\t\t(instance AND_%ld_%ld (viewRef %s (cellRef %s)))\n", level, i, current_library[cell].viewref, current_library[cell].cellref);
				}
				for (i = 0; i < level_pins / 2; i++) {
					fprintf(out, "\t\t\t\t(net NET_AND_%ld_%ld_A (joined\n", level, i);
					if (level) {
						fprintf(out, "\t\t\t\t\t(portRef %s (instanceRef AND_%ld_%ld))\n", current_library[cell].port_name[0], level - 1, 2 * i);
						fprintf(out, "\t\t\t\t\t(portRef %s (instanceRef AND_%ld_%ld))\n", current_library[cell].port_name[1], level, i);
					} else {
					...
					if (level) {
						if (i < (level_pins / 2)) {
							fprintf(out, "\t\t\t\t\t(portRef %s (instanceRef AND_%ld_%ld))\n", current_library[cell].port_name[0],  level - 1, 2 * i + 1);
						} else {
							fprintf(out, "\t\t\t\t\t(portRef %s (instanceRef AND_%ld_%ld))\n", current_library[cell].port_name[0], rightmost_pin_level, 2 * i + 1);
						}
						fprintf(out, "\t\t\t\t\t(portRef %s (instanceRef AND_%ld_%ld))\n", current_library[cell].port_name[2], level, i);
					} else {
						fprintf(out, "\t\t\t\t\t(portRef A%ld)\n", 2 * i + 2);
						fprintf(out, "\t\t\t\t\t(portRef %s (instanceRef AND_%ld_%ld))\n", current_library[cell].port_name[2], level, i);
					}
